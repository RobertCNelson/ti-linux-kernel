...
static void emc_ccfifo_writel(struct tegra_emc *tegra, u32 value,
			      unsigned long offset)
{
	writel(value, tegra->regs + EMC_CCFIFO_DATA);
	writel(offset, tegra->regs + EMC_CCFIFO_ADDR);
}
...
static void emc_seq_update_timing(struct tegra_emc *tegra)
{
	...
	for (i = 0; i < EMC_STATUS_UPDATE_TIMEOUT; ++i) {
		value = readl(tegra->regs + EMC_STATUS);
		if ((value & EMC_STATUS_TIMING_UPDATE_STALLED) == 0)
			return;
		udelay(1);
	}

	dev_err(tegra->dev, "timing update timed out\n");
}
...
static void emc_seq_disable_auto_cal(struct tegra_emc *tegra)
{
	...
	for (i = 0; i < EMC_STATUS_UPDATE_TIMEOUT; ++i) {
		value = readl(tegra->regs + EMC_AUTO_CAL_STATUS);
		if ((value & EMC_AUTO_CAL_STATUS_ACTIVE) == 0)
			return;
		udelay(1);
	}

	dev_err(tegra->dev, "auto cal disable timed out\n");
}
...
static void emc_seq_wait_clkchange(struct tegra_emc *tegra)
{
	...
	for (i = 0; i < EMC_STATUS_UPDATE_TIMEOUT; ++i) {
		value = readl(tegra->regs + EMC_INTSTATUS);
		if (value & EMC_INTSTATUS_CLKCHANGE_COMPLETE)
			return;
		udelay(1);
	}

	dev_err(tegra->dev, "clock change timed out\n");
}
				    ...
				    unsigned long rate)
{
	...
	__raw_writel(timing->emc_xm2dqspadctrl2,
		     tegra->regs + EMC_XM2DQSPADCTRL2);

	__raw_writel(timing->emc_zcal_interval,
		     tegra->regs + EMC_ZCAL_INTERVAL);
...
static void emc_debugfs_init(void)
{
	struct dentry *d;
