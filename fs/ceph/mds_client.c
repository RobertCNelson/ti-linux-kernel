...
static struct dentry *get_nonsnap_parent(struct dentry *dentry)
{
	 ...
	 */
	while (!IS_ROOT(dentry) && ceph_snap(fs_inode(dentry)) != CEPH_NOSNAP)
...
retry:
	...
	for (temp = dentry; !IS_ROOT(temp);) {
		struct inode *inode = fs_inode(temp);
			     ...
			     int *pfreepath)
{
	...
	if (ceph_snap(fs_inode(dentry->d_parent)) == CEPH_NOSNAP) {
		*pino = ceph_ino(fs_inode(dentry->d_parent));
					       ...
					       int mds)
{
	...
	if (req->r_inode_drop)
		releases += ceph_encode_inode_release(&p,
		      req->r_inode ? req->r_inode : fs_inode(req->r_dentry),
