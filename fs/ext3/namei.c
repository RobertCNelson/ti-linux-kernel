...
struct dentry *ext3_get_parent(struct dentry *child)
{
	...
	bh = ext3_find_entry(fs_inode(child), &dotdot, &de);
	if (!bh)
		return ERR_PTR(-ENOENT);
	ino = le32_to_cpu(de->inode);
	brelse(bh);

	if (!ext3_valid_inum(fs_inode(child)->i_sb, ino)) {
		ext3_error(fs_inode(child)->i_sb, "ext3_get_parent",
			   "bad inode number: %lu", ino);
		return ERR_PTR(-EIO);
	}

	return d_obtain_alias(ext3_iget(fs_inode(child)->i_sb, ino));
}
...
static int ext3_rmdir (struct inode * dir, struct dentry *dentry)
{
	...
	dquot_initialize(dir);
	dquot_initialize(fs_inode(dentry));
			   ...
			   struct inode * new_dir,struct dentry *new_dentry)
{
	...
	if (fs_inode(new_dentry))
		dquot_initialize(fs_inode(new_dentry));
