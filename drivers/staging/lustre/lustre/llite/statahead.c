			     ...
			     int entry_name_len)
{
	...
	if (!dentry) {
		...
		if (rc == 1 && agl_should_run(sai, fs_inode(dentry)))
			ll_agl_add(sai, fs_inode(dentry), entry->se_index);
	}
...
static void ll_start_agl(struct dentry *parent, struct ll_statahead_info *sai)
{
	...
	plli = ll_i2info(fs_inode(parent));
		       ...
		       int only_unplug)
{
		...
		if (entry->se_stat == SA_ENTRY_SUCC &&
			...
			if (rc == 1) {
				if (fs_inode(*dentryp) == NULL) {
					...
					*dentryp = alias;
				} else if (fs_inode(*dentryp) != inode) {
					/* revalidate, but inode is recreated */
					CDEBUG(D_READA,
					      "stale dentry %pd inode %lu/%u, statahead inode %lu/%u\n",
					      *dentryp,
					      fs_inode(*dentryp)->i_ino,
					      fs_inode(*dentryp)->i_generation,
					      inode->i_ino,
					      inode->i_generation);
					ll_sai_unplug(sai, entry);
					return -ESTALE;
	...
	if (unlikely(sai->sai_inode != fs_inode(parent))) {
		struct ll_inode_info *nlli = ll_i2info(fs_inode(parent));
