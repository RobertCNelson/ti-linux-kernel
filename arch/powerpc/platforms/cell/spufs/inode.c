...
static void spufs_prune_dir(struct dentry *dir)
{
	...
	mutex_lock(&fs_inode(dir)->i_mutex);
	list_for_each_entry_safe(dentry, tmp, &dir->d_subdirs, d_child) {
		spin_lock(&dentry->d_lock);
		if (!(d_unhashed(dentry)) && fs_inode(dentry)) {
			dget_dlock(dentry);
			__d_drop(dentry);
			spin_unlock(&dentry->d_lock);
			simple_unlink(fs_inode(dir), dentry);
...
static int spufs_rmdir(struct inode *parent, struct dentry *dir)
{
	...
	spu_forget(SPUFS_I(fs_inode(dir))->i_ctx);
	return res;
}
...
static int spufs_dir_close(struct inode *inode, struct file *file)
{
	...
	parent = fs_inode(dir->d_parent);
	ctx = SPUFS_I(fs_inode(dir))->i_ctx;
			...
			struct file *aff_filp)
{
	...
	if (affinity) {
		spufs_set_affinity(flags, SPUFS_I(fs_inode(dentry))->i_ctx,
								neighbor);
		if (neighbor)
			put_spu_context(neighbor);
	}
...
{
	struct inode *dir = fs_inode(path->dentry);
	int ret;
