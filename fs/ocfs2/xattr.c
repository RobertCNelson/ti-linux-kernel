			...
			size_t size)
{
	...
	struct ocfs2_inode_info *oi = OCFS2_I(fs_inode(dentry));
	...
	ret = ocfs2_inode_lock(fs_inode(dentry), &di_bh, 0);
	if (ret < 0) {
		mlog_errno(ret);
		return ret;
	}
	...
	i_ret = ocfs2_xattr_ibody_list(fs_inode(dentry), di, buffer, size);
	if (i_ret < 0)
		b_ret = 0;
	else {
		if (buffer) {
		...
		b_ret = ocfs2_xattr_block_list(fs_inode(dentry), di,
					       buffer, size);
		if (b_ret < 0)
			i_ret = 0;
	}
	up_read(&oi->ip_xattr_sem);
	ocfs2_inode_unlock(fs_inode(dentry), 0);
				    ...
				    void *buffer, size_t size, int type)
{
	if (strcmp(name, "") == 0)
		return -EINVAL;
	return ocfs2_xattr_get(fs_inode(dentry), OCFS2_XATTR_INDEX_SECURITY,
			       name, buffer, size);
}
		...
		const void *value, size_t size, int flags, int type)
{
	...
	return ocfs2_xattr_set(fs_inode(dentry), OCFS2_XATTR_INDEX_SECURITY,
		...
		void *buffer, size_t size, int type)
{
	if (strcmp(name, "") == 0)
		return -EINVAL;
	return ocfs2_xattr_get(fs_inode(dentry), OCFS2_XATTR_INDEX_TRUSTED,
			       name, buffer, size);
}
		...
		const void *value, size_t size, int flags, int type)
{
	...
	return ocfs2_xattr_set(fs_inode(dentry), OCFS2_XATTR_INDEX_TRUSTED,
			       name, value, size, flags);
}
		...
		void *buffer, size_t size, int type)
{
	...
	if (osb->s_mount_opt & OCFS2_MOUNT_NOUSERXATTR)
		return -EOPNOTSUPP;
	return ocfs2_xattr_get(fs_inode(dentry), OCFS2_XATTR_INDEX_USER, name,
			       buffer, size);
}
		...
		const void *value, size_t size, int flags, int type)
{
	...
	return ocfs2_xattr_set(fs_inode(dentry), OCFS2_XATTR_INDEX_USER,
			       name, value, size, flags);
}
