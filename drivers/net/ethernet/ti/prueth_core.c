...
static inline void prueth_ptp_ts_enable(struct prueth_emac *emac)
{
	...
	if (emac->ptp_tx_enable) {
		val = readb(sram + TIMESYNC_CTRL_VAR_OFFSET);
...
static int prueth_ptp_tx_ts_enqueue(struct prueth_emac *emac, struct sk_buff *skb)
{
	...
	if (eth_hdr && eth_hdr->h_proto == htons(ETH_P_HSR)) {
...
/**
 * emac_ndo_get_stats - EMAC get statistics function
 * @ndev: The EMAC network adapter
 *
 ...
 * @features: 64 bit unsigned int whose each bit indicates a feature
			      ...
			      struct device_node *eth_node)
{
	...
	ret = of_get_mac_address(eth_node, mac_addr);
	if (!ret)
	...
	if (!is_valid_ether_addr(mac_addr)) {
		...
		ether_addr_copy(emac->mac_addr, mac_addr);
	}
