...
static int hppfs_readlink(struct dentry *dentry, char __user *buffer,
			  int buflen)
{
	struct dentry *proc_dentry = HPPFS_I(fs_inode(dentry))->proc_dentry;
	return fs_inode(proc_dentry)->i_op->readlink(proc_dentry, buffer,
						    buflen);
}
...
static void *hppfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	return fs_inode(proc_dentry)->i_op->follow_link(proc_dentry, nd);
}
			   ...
			   void *cookie)
{
	...
	if (fs_inode(proc_dentry)->i_op->put_link)
		fs_inode(proc_dentry)->i_op->put_link(proc_dentry, nd, cookie);
}
...
{
	struct inode *proc_ino = fs_inode(dentry);
