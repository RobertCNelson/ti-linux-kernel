	...
	int aptpl)
{
	...
	list_for_each_entry_safe(lun_tmp, next, &dev->dev_sep_list, lun_dev_link) {
		...
		list_for_each_entry(deve_tmp, &lun_tmp->lun_deve_list, lun_link) {
			...
			lacl_tmp = lockless_dereference(deve_tmp->se_lun_acl);
			...
			pr_reg_atp = __core_scsi3_do_alloc_registration(dev,
						nacl_tmp,
						lockless_dereference(deve_tmp->se_lun),
...
static int core_scsi3_lunacl_depend_item(struct se_dev_entry *se_deve)
{
	 ...
	 */
	lun_acl = lockless_dereference(se_deve->se_lun_acl);
	if (!lun_acl)
		return 0;
	...
	int aptpl)
{
	...
	while (tpdl > 0) {
		...
		dest_pr_reg = __core_scsi3_alloc_registration(cmd->se_dev,
					dest_node_acl,
					lockless_dereference(dest_se_deve->se_lun),
