		...
		u32 sgl_nents, int is_write)
{
	...
	if (is_write) {
		iov_iter_bvec(&iter, ITER_BVEC | WRITE, bvec, sgl_nents, len);
		ret = vfs_iter_write(fd, &iter, &pos);
	} else {
		iov_iter_bvec(&iter, ITER_BVEC | READ, bvec, sgl_nents, len);
...
fd_execute_write_same(struct se_cmd *cmd)
{
	...
	iov_iter_bvec(&iter, ITER_BVEC | WRITE, bvec, nolb, len);
