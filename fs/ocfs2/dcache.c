...
{
	unsigned long gen =
		OCFS2_I(fs_inode(dentry->d_parent))->ip_dir_lock_gen;
...
static int ocfs2_dentry_revalidate(struct dentry *dentry, unsigned int flags)
{
	...
	if (inode == NULL) {
		...
		pgen = OCFS2_I(fs_inode(dentry->d_parent))->ip_dir_lock_gen;
			      ...
			      int skip_unhashed)
{
	...
	parent = fs_inode(dentry->d_parent);
			     ...
			     u64 parent_blkno)
{
	...
	if (!fs_inode(dentry) && dentry->d_fsdata) {
