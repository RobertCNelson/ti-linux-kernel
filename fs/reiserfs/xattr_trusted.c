	    ...
	    int handler_flags)
{
	...
	if (!capable(CAP_SYS_ADMIN) || IS_PRIVATE(fs_inode(dentry)))
		return -EPERM;

	return reiserfs_xattr_get(fs_inode(dentry), name, buffer, size);
}
	    ...
	    size_t size, int flags, int handler_flags)
{
	...
	return reiserfs_xattr_set(fs_inode(dentry), name, buffer, size, flags);
}
