	      ...
	      struct dentry *direntry)
{
	 ...
	 */
	if (fs_inode(old_file)) {
		cifsInode = CIFS_I(fs_inode(old_file));
		if (rc == 0) {
			spin_lock(&fs_inode(old_file)->i_lock);
			inc_nlink(fs_inode(old_file));
			spin_unlock(&fs_inode(old_file)->i_lock);
