			     ...
			     unsigned int size, int excl, int *opened)
{
	...
	if ((GFS2_I(fs_inode(sdp->sd_root_dir)) == dip) ||
		       ...
		       struct inode *ndir, struct dentry *ndentry)
{
	...
	struct gfs2_inode *ip = GFS2_I(fs_inode(odentry));
	...
	if (dir_rename) {
		error = gfs2_permission(fs_inode(odentry), MAY_WRITE);
		if (error)
			goto out_gunlock;
	}
...
{
	struct gfs2_inode *ip = GFS2_I(fs_inode(dentry));
	struct gfs2_holder i_gh;
	struct buffer_head *dibh;
	unsigned int size;
	char *buf;
