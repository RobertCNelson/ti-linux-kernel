...
static int read_rdev(struct mddev *mddev, struct md_rdev *rdev)
{
	...
	sb = page_address(rdev->sb_page);
	/* Do not set recovery_offset back to zero from MaxSector
	 * This would ensure that when this node does the recovery, we
	 * would set mddev->degraded right.
	 */
	if (rdev->recovery_offset != MaxSector || le64_to_cpu(sb->recovery_offset) != 0)
			rdev->recovery_offset = le64_to_cpu(sb->recovery_offset);
	put_page(swapout);
	return 0;
}
