...
static int jfs_rmdir(struct inode *dip, struct dentry *dentry)
{
	int rc;
	tid_t tid;		/* transaction id */
	struct inode *ip = fs_inode(dentry);
	ino_t ino;
	     ...
	     struct inode *dir, struct dentry *dentry)
{
	int rc;
	tid_t tid;
	struct inode *ip = fs_inode(old_dentry);
	ino_t ino;
	struct component_name dname;
	struct btstack btstack;
	struct inode *iplist[2];
	       ...
	       struct inode *new_dir, struct dentry *new_dentry)
{
	...
	old_ip = fs_inode(old_dentry);
	new_ip = fs_inode(new_dentry);
...
struct dentry *jfs_get_parent(struct dentry *dentry)
{
	...
	parent_ino =
		le32_to_cpu(JFS_IP(fs_inode(dentry))->i_dtroot.header.idotdot);

	return d_obtain_alias(jfs_iget(fs_inode(dentry)->i_sb, parent_ino));
}
