...
/* Retrieve the rproc platform id. The platform id for various
 * remoteprocs are currently assumed to be fixed, and this provides
...
static int rpmsg_sock_get_proc_id(struct rpmsg_channel *rpdev)
{
	...
	id = rproc_get_platform_id(rproc);
	WARN_ON(id < 0);
...
#if 0 /* left in place only for debugging sake */
