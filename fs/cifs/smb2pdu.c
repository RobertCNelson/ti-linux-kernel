...
static void
build_preauth_ctxt(struct smb2_preauth_neg_context_req *pneg_ctxt)
{
	pneg_ctxt->ContextType = SMB2_PREAUTH_INTEGRITY_CAPABILITIES;
	pneg_ctxt->DataLength = cpu_to_le16(38);
	pneg_ctxt->HashAlgorithmCount = cpu_to_le16(1);
	pneg_ctxt->SaltLength = cpu_to_le16(32);
	/* FIXME create salt */
	pneg_ctxt->HashAlgorithms = SMB2_PREAUTH_INTEGRITY_SHA512;
}

static void
build_encrypt_ctxt(struct smb2_encryption_neg_context_req *pneg_ctxt)
{
...
assemble_neg_contexts(struct smb2_negotiate_req *req)
{
	...
	pneg_ctxt += 2 + sizeof(struct smb2_preauth_neg_context_req);
	build_encrypt_ctxt((struct smb2_encryption_neg_context_req *)pneg_ctxt);
	req->NegotiateContextOffset = cpu_to_le32(OFFSET_OF_NEG_CONTEXT);
	req->NegotiateContextCount = cpu_to_le16(2);
	inc_rfc1001_len(req, 4 + sizeof(struct smb2_preauth_neg_context_req) + 2
			+ sizeof(struct smb2_encryption_neg_context_req));
}
#else
