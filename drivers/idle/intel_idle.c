...
static void bxt_idle_state_table_update(void)
{
	...
	if (msr)
		bxt_cstates[2].exit_latency = irtl_2_usec(msr);

	rdmsrl(MSR_PKGC7_IRTL, msr);
	if (msr)
		bxt_cstates[3].exit_latency = irtl_2_usec(msr);

	rdmsrl(MSR_PKGC8_IRTL, msr);
	if (msr)
		bxt_cstates[4].exit_latency = irtl_2_usec(msr);

	rdmsrl(MSR_PKGC9_IRTL, msr);
	if (msr)
		bxt_cstates[5].exit_latency = irtl_2_usec(msr);

	rdmsrl(MSR_PKGC10_IRTL, msr);
	if (msr)
		bxt_cstates[6].exit_latency = irtl_2_usec(msr);
