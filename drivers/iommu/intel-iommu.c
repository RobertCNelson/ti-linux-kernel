			 ...
			 u8 bus, u8 devfn)
{
	...
	if (ecs_enabled(iommu)) {
		...
		if (ecap_prs(iommu->ecap) && pci_find_ext_capability(pdev, PCI_EXT_CAP_ID_PRI))
	...
	if (ecap_dev_iotlb_support(iommu->ecap) &&
		...
		else if (intel_iommu_pasid28 && IS_GFX_DEVICE(pdev) &&
			 pdev->vendor == 0x8086) {
...
int intel_iommu_enable_pasid(struct intel_iommu *iommu, struct intel_svm_dev *sdev)
{
	...
	if (WARN_ON(device_to_iommu(sdev->dev, &bus, &devfn) != iommu))
		return -EINVAL;
	...
	if (!(ctx_lo & CONTEXT_PASIDE)) {
		...
		context[0].lo = ctx_lo;
		wmb();
		iommu->flush.flush_context(iommu, sdev->did,
					   (((u16)bus) << 8) | devfn,
					   DMA_CCMD_MASK_NOBIT,
					   DMA_CCMD_DEVICE_INVL);
	}
