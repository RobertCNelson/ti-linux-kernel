...
{
	struct task_struct *task = get_proc_task(fs_inode(dentry));
	int result = -ENOENT;
...
static int proc_exe_link(struct dentry *dentry, struct path *exe_path)
{
	...
	task = get_proc_task(fs_inode(dentry));
	if (!task)
		return -ENOENT;
	mm = get_task_mm(task);
	put_task_struct(task);
...
int pid_delete_dentry(const struct dentry *dentry)
{
	 ...
	 * kill it immediately.
	 */
	return proc_inode_is_dead(fs_inode(dentry));
}
	...
	instantiate_t instantiate, struct task_struct *task, const void *ptr)
{
	...
	if (!child) {
		...
		if (instantiate(fs_inode(dir), child, task, ptr) < 0) {
			dput(child);
			goto end_instantiate;
		}
	}
	inode = fs_inode(child);
	ino = inode->i_ino;
...
int proc_pid_readdir(struct file *file, struct dir_context *ctx)
{
	...
	if (pos == TGID_OFFSET - 2) {
		struct inode *inode = fs_inode(ns->proc_self);
		if (!dir_emit(ctx, "self", 4, inode->i_ino, DT_LNK))
			return 0;
		ctx->pos = pos = pos + 1;
	}
	if (pos == TGID_OFFSET - 1) {
		struct inode *inode = fs_inode(ns->proc_thread_self);
