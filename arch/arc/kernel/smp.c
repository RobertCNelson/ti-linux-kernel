...
/*
 * Default smp boot helpers (master:kick, non-master:wait) for simple
 * debugger assisted boot (NON Halt-on-reset system)
 *
 * Halt-on-reset
 * - Reset Vector has a small trampoline which makes master jump to kernel's
 *   main entry point @stext, others jump off to @first_lines_of_secondary
 * - Needs platform to implement smp_ops.cpu_kick for master to take the
 *   other out of reset.
 * - arc_platform_smp_wait_to_boot() is NOT needed at all
 *
 * NON Halt-on-reset
 * - All cores start off together @ stext (w/o Reset Vector coming into picture)
 * - Determine cpu-ID (o..n): Master (0) jumps proceeds with boot
 * - Others spin in arc_platform_smp_wait_to_boot() waiting on @wake_flag
 * - One Master is ready, it loops thru all slaves and sets @wake_flag
 *   enabling one core at a time to procced, which jumps off to
