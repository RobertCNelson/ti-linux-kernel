...
static int __must_check __deliver_machine_check(struct kvm_vcpu *vcpu)
{
	 ...
	 * with it, so always clear bits for repressible and exigent interrupts
	 */
	clear_bit(IRQ_PEND_MCHK_EX, &li->pending_irqs);
	clear_bit(IRQ_PEND_MCHK_REP, &li->pending_irqs);
	clear_bit(IRQ_PEND_MCHK_REP, &fi->pending_irqs);
...
{
	struct kvm_s390_float_interrupt *fi = &vcpu->kvm->arch.float_int;
	struct kvm_s390_interrupt_info *n, *inti;
	int deliver = 0;
	int rc = 0;

	spin_lock(&fi->lock);
	list_for_each_entry_safe(inti, n, &fi->lists[FIRQ_LIST_PFAULT], list) {
		trace_kvm_s390_deliver_interrupt(vcpu->vcpu_id,
...
static int __must_check __deliver_virtio(struct kvm_vcpu *vcpu)
{
	...
	spin_lock(&fi->lock);
	list_for_each_entry_safe(inti, n, &fi->lists[FIRQ_LIST_VIRTIO], list) {
			VCPU_EVENT(vcpu, 4,
				     ...
				     unsigned long irq_type)
{
	...
	list_for_each_entry_safe(inti, n, isc_list, list) {
...
static struct kvm_s390_interrupt_info *get_io_inti(struct kvm_s390_float_interrupt *fi,
						   int isc, u64 schid)
{
	struct kvm_s390_interrupt_info *iter;

	list_for_each_entry(iter, &fi->lists[FIRQ_LIST_IO_ISC_0 + isc], list) {
		if (!schid)
			return iter;
						    ...
						    u64 cr6, u64 schid)
{
	...
	for (isc = 0; isc <= MAX_ISC && !inti; isc++) {
		if (!(cr6 & isc_to_isc_bits(isc)))
			continue;
		spin_lock(&fi->lock);
		inti = get_io_inti(fi, isc, schid);
		if (!inti) {
			spin_unlock(&fi->lock);
			break;
		}
...
#define SCCB_MASK 0xFFFFFFFE
#define SCCB_EVENT_PENDING 0x1
			     ...
			     struct kvm_s390_interrupt_info *inti)
{
	 ...
	 * We will silently ignore those superfluos interrupts.
	 * A future version of QEMU will take care of serialization
	 * of servc requests
	 */
	if (inti->ext.ext_params & fi->srv_signal.ext_params &
	    SCCB_MASK) {
		spin_unlock(&fi->lock);
		return 0;
	}
	/*
	 * If we already have a pending service interrupt and another
	 * one is injected, only update the event pending bit
	 */
	if (fi->srv_signal.ext_params & SCCB_MASK)
		fi->srv_signal.ext_params |= (inti->ext.ext_params &
					      SCCB_EVENT_PENDING);
	else
		fi->srv_signal.ext_params |= inti->ext.ext_params;
				...
				struct kvm_s390_interrupt_info *inti)
{
	...
	spin_lock(&fi->lock);
	if (fi->pending_irqs & (1UL << IRQ_PEND_MCHK_REP)) {
		spin_unlock(&fi->lock);
		return -EBUSY;
	}
	fi->mchk = inti->mchk;
