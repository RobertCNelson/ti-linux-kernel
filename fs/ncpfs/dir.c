...
{
	struct inode *inode = fs_inode_once(dentry);
                 ...
                 struct inode *new_dir, struct dentry* new_dentry, char *_new_name)
{
	...
	if (new_dentry && fs_inode(new_dentry)) {
		new_nwattr = NCP_FINFO(fs_inode(new_dentry))->nwattr;
...
{
	struct ncp_server *server = NCP_SERVER(fs_inode(parent));
	...
	for (;;) {
		...
		while (ctl.idx < NCP_DIRCACHE_SIZE) {
			...
			over = !dir_emit(ctx, dent->d_name.name,
					dent->d_name.len,
					fs_inode(dent)->i_ino, DT_UNKNOWN);
			dput(dent);
			if (over)
				goto finished;
			ctx->pos += 1;
...
static void ncp_d_prune(struct dentry *dentry)
{
	if (!dentry->d_fsdata)	/* not referenced from page cache */
		return;
	NCP_FINFO(fs_inode(dentry->d_parent))->flags &= ~NCPI_DIR_CACHE;
}
		...
		int inval_childs)
{
	...
	if (!fs_inode(newdent)) {
		...
		struct inode *inode = fs_inode(newdent);
...
int ncp_conn_logged_in(struct super_block *sb)
{
	...
	if (ncp_single_volume(server)) {
		...
		dent = sb->s_root;
		if (dent) {
			struct inode* ino = fs_inode(dent);
			if (ino) {
				...
				ncp_dbg(1, "fs_inode(sb->s_root) == NULL!\n");
			}
		} else {
