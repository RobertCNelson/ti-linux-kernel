...
/*
 * If fs_inode(dentry) is null (usually meaning the cached dentry
...
int cifs_rmdir(struct inode *inode, struct dentry *direntry)
{
	...
	if (!rc) {
		spin_lock(&fs_inode(direntry)->i_lock);
		i_size_write(fs_inode(direntry), 0);
		clear_nlink(fs_inode(direntry));
		spin_unlock(&fs_inode(direntry)->i_lock);
	}

	cifsInode = CIFS_I(fs_inode(direntry));
	...
	fs_inode(direntry)->i_ctime = inode->i_ctime = inode->i_mtime =
...
unlink_target:
	...
	if (fs_inode(target_dentry) && (rc == -EACCES || rc == -EEXIST)) {
...
cifs_setattr_unix(struct dentry *direntry, struct iattr *attrs)
{
	...
	unsigned int xid;
	char *full_path = NULL;
	struct inode *inode = fs_inode(direntry);
	struct cifsInodeInfo *cifsInode = CIFS_I(inode);
	struct cifs_sb_info *cifs_sb = CIFS_SB(inode->i_sb);
	struct tcon_link *tlink;
