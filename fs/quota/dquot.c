		   ...
		   struct path *path)
{
	...
	if (path->dentry->d_sb != sb)
		...
		error = vfs_load_quota_inode(fs_inode(path->dentry), type,
		...
		int format_id, int type)
{
	...
	mutex_lock(&fs_inode(sb->s_root)->i_mutex);
	dentry = lookup_one_len(qf_name, sb->s_root, strlen(qf_name));
	mutex_unlock(&fs_inode(sb->s_root)->i_mutex);
	if (IS_ERR(dentry))
	...
	if (!error)
		error = vfs_load_quota_inode(fs_inode(dentry), type, format_id,
				DQUOT_USAGE_ENABLED | DQUOT_LIMITS_ENABLED);
