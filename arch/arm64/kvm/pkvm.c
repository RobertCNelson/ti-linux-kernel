...
void pkvm_pgtable_stage2_destroy(struct kvm_pgtable *pgt)
{
	...
	while (node) {
		mapping = rb_entry(node, struct pkvm_mapping, node);
		kvm_call_hyp_nvhe(__pkvm_host_unshare_guest, handle, mapping->gfn, 0);
		node = rb_next(node);
			   ...
			   void *mc, enum kvm_pgtable_walk_flags flags)
{
	...
	ret = kvm_call_hyp_nvhe(__pkvm_host_share_guest, pfn, gfn, prot, 1);
	if (ret) {
...
int pkvm_pgtable_stage2_unmap(struct kvm_pgtable *pgt, u64 addr, u64 size)
{
	...
	for_each_mapping_in_range_safe(pgt, addr, addr + size, mapping) {
		ret = kvm_call_hyp_nvhe(__pkvm_host_unshare_guest, handle, mapping->gfn, 0);
		if (WARN_ON(ret))
			break;
		rb_erase(&mapping->node, &pgt->pkvm_mappings);
		kfree(mapping);
...
int pkvm_pgtable_stage2_wrprotect(struct kvm_pgtable *pgt, u64 addr, u64 size)
{
	...
	for_each_mapping_in_range_safe(pgt, addr, addr + size, mapping) {
		ret = kvm_call_hyp_nvhe(__pkvm_host_wrprotect_guest, handle, mapping->gfn, 0);
		if (WARN_ON(ret))
			break;
	}
...
{
	return kvm_call_hyp_nvhe(__pkvm_host_relax_perms_guest, addr >> PAGE_SHIFT, prot, 0);
}
