		 ...
		 size_t value_len, int flags)
{
	...
	rc = __jfs_setxattr(tid, fs_inode(dentry), name, value, value_len,
			    flags);
	if (!rc)
		     ...
		     size_t buf_size)
{
	...
	err = __jfs_getxattr(fs_inode(dentry), name, data, buf_size);
...
int jfs_removexattr(struct dentry *dentry, const char *name)
{
	...
	tid = txBegin(inode->i_sb, 0);
	mutex_lock(&ji->commit_mutex);
	rc = __jfs_setxattr(tid, fs_inode(dentry), name, NULL, 0, XATTR_REPLACE);
	if (!rc)
		rc = txCommit(tid, 1, &inode, 0);
	txEnd(tid);
	mutex_unlock(&ji->commit_mutex);
